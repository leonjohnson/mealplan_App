...
open class AKPickerView: UIView, AKPickerViewDataSource,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    ...
    var divider   = 2
    var endIcon  = ""
    var dividersBetween = 1;
    
    open func numberOfItemsInPickerView(_ pickerView: AKPickerView) -> Int{
        return ((endPoint - startPoint) * divider ) + ( dividersBetween);
    }
    
    open func pickerView(_ pickerView: AKPickerView, titleForItem item: Int) -> String{
        
        if(((item % divider) != 0 && seperator == "")||(item % (dividersBetween + 1)) != 0 ){
            
            return  ""
            
        }else if((item % divider) != 0 && seperator != ""){
            
            return getValue1(item).description + seperator + getValue2(item).description + endIcon
            
        }
        
        return getValue1(item).description;
    }
    open func getValue1(_ item:Int) -> Int{
        return (startPoint + (item/divider))
    }
    open func getValue2(_ item:Int)-> Int{
        return ((item % divider))
    }
    open func pickerView(_ pickerView: AKPickerView, imageForItem item: Int) -> UIImage{
    ...
    @IBInspectable var newDevider: Int = 2   {
        didSet{
            divider = newDevider
        }
        
    }
    @IBInspectable var newSeperator: String = ""   {
        didSet{
            seperator = newSeperator
        }
        
    }
    @IBInspectable var newEndIcon: String = "."   {
        didSet{
            endIcon = newEndIcon
        }
    ...
    open lazy var font = UIFont.systemFont(ofSize: 12)
    
    /// Readwrite. A font which used in selected cells.
    open lazy var highlightedFont = UIFont.boldSystemFont(ofSize: 12)
